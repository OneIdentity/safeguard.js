...
uglifyjs --compress --mangle --toplevel .\src\safeguardWeb.js | Out-File -FilePath .\src\safeguardWeb.min.js -Encoding utf8
...
foreach ($dir in Get-ChildItem -Path samples/browser/Promises){
   copy src/safeguardweb.min.js samples/browser/Promises/$dir
...
foreach ($dir in Get-ChildItem -Path samples/browser/Callbacks){
   copy src/safeguardweb.min.js samples/browser/Callbacks/$dir
